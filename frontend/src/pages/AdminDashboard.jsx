  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <div key={s.label} className={`bg-gradient-to-br ${s.color} border rounded-xl p-3 text-center`}>
            ...
            <p className="text-xl font-bold mt-1 text-gray-700 leading-relaxed">{s.value}</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-soft p-4">
        <h3 className="text-lg mb-3 text-gray-900 font-bold tracking-tight">System Overview Map</h3>
        <MapView
          center={position}
          donations={donations}
          requests={requests}
        ...
        <div className="p-4 overflow-x-auto">
          {activeTab === 'donations' && (
            <table className="w-full text-sm">
              ...
              <tbody className="text-slate-700">
                {donations.map((d) => (
                  <tr key={d._id} className="border-b border-gray-300 hover:bg-slate-100/20">
                    <td className="py-2 px-2 font-medium">{d.title}</td>
                    <td className="py-2 px-2 text-gray-500">{d.donorId?.name || '—'}</td>
                    <td className="py-2 px-2">{d.quantity}</td>
                    <td className="py-2 px-2"><StatusBadge status={d.status} /></td>
                    <td className="py-2 px-2 text-gray-500">{d.assignedVolunteer?.name || '—'}</td>
                    <td className="py-2 px-2 text-slate-400 text-xs">{new Date(d.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {activeTab === 'requests' && (
            <table className="w-full text-sm">
              ...
              <tbody className="text-slate-700">
                {requests.map((r) => (
                  <tr key={r._id} className="border-b border-gray-300 hover:bg-slate-100/20">
                    <td className="py-2 px-2 font-medium">{r.foodNeeded}</td>
                    <td className="py-2 px-2 text-gray-500">{r.ngoId?.name || '—'}</td>
                    ...
                    <td className="py-2 px-2 text-slate-400 text-xs">{new Date(r.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody className="text-slate-700">
                {deliveries.map((d) => (
                  <tr key={d._id} className="border-b border-gray-300 hover:bg-slate-100/20">
                    ...
                    <td className="py-2 px-2 text-gray-500">{d.volunteerId?.name || '—'}</td>
